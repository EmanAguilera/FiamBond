...
// --- "AI ADVISOR" COMPONENT (Kept as is) ---
const FinancialAnalysis = ({ report }) => {
    ...
    if (netPosition > 0 && totalInflow > totalOutflow * 1.2) { title = "Excellent Financial Discipline"; narrative = `You demonstrated outstanding control over your finances, generating a strong positive cash flow of ${netFormatted}.`; recommendation = "Consider allocating this surplus towards your goals. Maintain this great momentum!";
    } else if (netPosition >= 0) { title = "Good Financial Management"; narrative = `You successfully kept your spending in check, resulting in a positive net position of ${netFormatted}.`; recommendation = "Challenge yourself to find small areas to reduce spending and further increase your savings rate next period.";
    } else { title = "Spending Review Recommended"; narrative = `Your expenses exceeded your income, resulting in a net outflow of ${netFormatted}. It's important to understand why.`; recommendation = `Review your ${transactionCount} transactions from this period to identify the cause. Focus on tracking your outflow to stay within your income going forward.`; }
    return (<div className="mt-8"><h3 className="text-lg font-bold text-gray-800">Analyst's Summary</h3><div className="mt-2 p-4 bg-gray-50 rounded-lg border"><p className="font-semibold text-gray-800">{title}</p><p className="mt-1 text-sm text-gray-600">{narrative}</p><p className="mt-3 text-sm font-semibold text-gray-800">Next Steps:</p><p className="mt-1 text-sm text-gray-600">{recommendation}</p></div></div>);
};
...
function PersonalReportChartWidget({ report }) {
    ...
    if (!report) { return (<div className="dashboard-card text-center py-10"><p className="text-gray-500 italic">No report data available.</p></div>); }
    const hasChartData = report.chartData?.datasets?.length > 0 && report.chartData?.labels?.length > 0;

    return (
        // --- THE FIX: We use a white 'dashboard-card' as a container ---
        <div className="dashboard-card p-4 sm:p-6">
            ...
            {/* --- STAT CARDS WITH CORRECTED COLOR PALETTE --- */}
            ...
            {/* --- CHART SECTION --- */}
            <div className="mt-8">
                <h3 className="text-lg font-bold text-gray-800 mb-2">Inflow vs. Outflow Breakdown</h3>
                <div className="relative" style={{ height: '300px' }}>
                    {hasChartData ? (
                        // Bar component relies on ChartJS, making this strictly client-side
                        ...
                        <div className="flex flex-col items-center justify-center h-full bg-gray-50 rounded-lg">
                            ...
                            <p className="text-xs text-gray-400">Add more transactions to see a breakdown.</p>
                        </div>
                    )}
                </div>
            </div>
