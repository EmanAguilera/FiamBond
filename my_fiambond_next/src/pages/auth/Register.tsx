...
// Ensure this path matches your firebase-config location
...
function RegisterContent() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-12">
      {/* ⭐️ Render overlay when submitting using the 'fullscreen' type */}
      {isSubmitting && (
        <UnifiedLoadingWidget type="fullscreen" message="Creating Account..." variant="indigo" />
      )}
      ...
      <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
        ...
        <form onSubmit={handleRegisterSubmit} className="space-y-4">
          ...
          <button 
            className={`w-full bg-indigo-600 text-white py-3 rounded-lg font-bold shadow-md hover:bg-indigo-700 transition-all ${!auth || isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
            type="submit"
            disabled={!auth || isSubmitting}
          >
            Create Account
...
export default function Register() {
  return (
    // ⭐️ Initial Suspense load using your new widget type system
