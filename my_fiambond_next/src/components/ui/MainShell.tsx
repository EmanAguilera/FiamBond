...
const AppFooter = () => (
    <footer className="bg-white py-12 border-t border-gray-200">
        <div className="max-w-screen-2xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            {/* ⭐️ ADDED: About Us link in the middle nav */}
...
export default function MainShell({ children }: { children: React.ReactNode }) {
    ...
    // FIX: Force pathname to be a string immediately to satisfy TypeScript
    const pathname = usePathname() ?? '';

    // Logic for public layouts
    const publicLayoutPaths = ['/', '/login', '/register', '/verify-email', '/terms', '/privacy', '/welcome'];
