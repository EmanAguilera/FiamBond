...
const SubscriptionTicker = ({ label, type, user, onRevoke }) => {
    ...
    // Format End Date to show Date AND Time (e.g., 12/17/2025 5:30 PM)
...
export const AdminUserRow = memo(({ user, rightContent, onTogglePremium }) => {
    ...
    const approvalButtons = (type, label) => (
        <div className="flex flex-col items-end gap-2">
            ...
            <button onClick={() => { if(confirm("Reject?")) onTogglePremium(user.id, 'reject', type); }} className="text-xs text-rose-500 hover:text-rose-700 font-medium px-2">Reject</button>
        </div>
    );
