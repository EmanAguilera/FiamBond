'use client'; // Required due to the use of useState, useContext, useRef, and browser APIs (fetch, File, FormData)

import { useState, useContext, useRef } from "react";
import Tesseract from 'tesseract.js'; // Client-side library
...
export default function ApplyPremiumWidget({ onClose, onUpgradeSuccess, targetAccess = 'company' }) {
    ...
    const processScannedText = (text) => {
        ...
        // Regex for common 10-13 digit reference numbers (adjust as needed)
        const refMatch = cleanText.match(/(\d{10,13})/); 
        
        if (refMatch) {
            setRefNumber(refMatch[1]);
        } else {
            // Fallback for 8-9 digits if 10-13 fails
