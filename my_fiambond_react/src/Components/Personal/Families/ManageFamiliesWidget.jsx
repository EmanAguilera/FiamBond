...
const ManageFamiliesWidget = ({ onEnterRealm }) => {
    ...
    const checkHasTransactions = async (familyId) => {
        try {
            ...
            // Default to safe side: if check fails, assume risk exists or allow? 
            // Usually safest to allow retry, but here we return false to prevent UI blocking on network error
            return false; 
        }
    };
    ...
    // --- RESTORED LOGIC: RENAME WITH VALIDATION ---
    ...
    // --- RESTORED LOGIC: DELETE WITH VALIDATION ---
