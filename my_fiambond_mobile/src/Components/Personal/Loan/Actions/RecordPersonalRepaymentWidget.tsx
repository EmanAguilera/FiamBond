...
interface Loan {
    ...
    description: string;
    total_owed?: number | string; // Updated to include string|number possibility
...
export default function RecordPersonalRepaymentWidget({ loan, onSuccess }: RecordPersonalRepaymentWidgetProps) {
    ...
    const handleRecordRepayment = async () => {
        ...
        if (repaymentAmount > outstanding) {
            ...
            Alert.alert("Error", `Amount cannot exceed the outstanding balance of ₱${outstanding.toFixed(2)}.`);
            return;
        }
        ...
        try {
            ...
            // FIX: Convert totalOwed to a number before comparison (Error 2365)
            const numericTotalOwed = Number(totalOwed); 
            const newStatus = newRepaidAmount >= numericTotalOwed ? "repaid" : "outstanding";
            ...
            // 4. POST Transaction (Record Income for Creditor - User)
            ...
            if (!creditorTxResponse.ok) console.warn("Loan updated, but failed to record creditor income transaction.");
            ...
            Alert.alert("Success", `Repayment of ₱${repaymentAmount.toFixed(2)} recorded successfully.`);
            onSuccess();

        } catch (err: any) {
            console.error("Failed to record repayment:", err);
            setError("Could not record the repayment. Please try again.");
            Alert.alert("Error", "Could not record the repayment. Please try again.");
        } finally {
            setLoading(false);
    ...
    return (
        <View style={styles.container}>
            <View>
                <Text style={styles.promptText}>You are recording a repayment received from:</Text>
                <Text style={styles.loanDescription}>{debtorDisplayName}</Text>
            ...
            <View>
                <Text style={styles.label}>
                    Attach Proof of Receipt <Text style={styles.labelOptional}>(Optional)</Text>
                </Text>
                <TouchableOpacity 
            ...
            <Text style={styles.infoText}>This will be added to your personal balance as income.</Text>
        </View>
    );
}
