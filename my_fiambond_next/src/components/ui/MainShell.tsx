...
export default function MainShell({ children }: { children: React.ReactNode }) {
    ...
    const publicLayoutPaths = ['/', '/login', '/register', '/verify-email', '/terms', '/privacy', '/welcome', '/about-us'];
    const isPublicLayout = !user || publicLayoutPaths.includes(pathname);
    ...
    const currentPremiumDetails = useMemo(() => {
        ...
        const details: any = {};
        if (user.is_premium) {
            details.company = { granted_at: user.premium_granted_at, plan_cycle: user.premium_plan_cycle || 'monthly' };
        }
        if (user.is_family_premium) {
            details.family = { granted_at: user.family_premium_granted_at, plan_cycle: user.family_premium_plan_cycle || 'monthly' };
        }
        return (details.company || details.family) ? details : null;
    }, [user]);

    const isAdmin = user?.role === 'admin';

    // --- VIEW A: PUBLIC LAYOUT ---
    if (isPublicLayout && !pathname.startsWith('/realm') && !pathname.startsWith('/settings')) { 
        return (
                    ...
                    <div className="max-w-screen-2xl flex items-center justify-between mx-auto p-4 px-6 lg:px-8 h-full">
                        ...
                        <div className="flex items-center gap-4">
                            {!user ? (
                                ...
                                <Link href="/realm" className="bg-indigo-600 text-white px-6 py-2.5 rounded-lg font-bold text-sm hover:bg-indigo-700 transition-all">Go to Realm</Link>
                            )}
                        </div>
                    </div>
                </nav>
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="hidden md:block relative" ref={dropdownRef}>
                            ...
                            >
                                ...
                                {/* ⭐️ FIXED: Changed from <button> to <div> to prevent nesting error */}
                                <div className={`h-4 w-4 text-gray-400 transition-transform ${isProfileMenuOpen ? 'rotate-180' : ''}`}>
                                    <ChevronDownIcon />
                                </div>
                            </button>
                    ...
                    <div className="md:hidden border-t border-gray-200 bg-white shadow-lg">
                        <div className="px-4 py-6 space-y-6">
                             ...
                             <nav className="flex flex-col gap-2">
                                <Link href="/settings" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 p-3 font-medium text-gray-700 hover:bg-gray-50 rounded-lg">
                                    <Cog6ToothIcon className="h-5 w-5" /> Settings
                                </Link>
                                <button onClick={handleLogout} className="flex items-center gap-3 p-3 font-medium text-red-600 hover:bg-red-50 rounded-lg text-left">
