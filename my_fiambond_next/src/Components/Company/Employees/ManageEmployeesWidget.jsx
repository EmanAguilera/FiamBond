        ...
        <form onSubmit={handleSubmit} className="p-4 bg-white rounded-lg border border-slate-100 shadow-sm mt-2">
            ...
            <div className="mb-4">
                <input 
                    ...
                    className="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-800" 
...
export default function ManageEmployeesWidget({ company, members, onUpdate }) {
    ...
    const handleAddEmployee = async (email) => {
        try {
            ...
            // 2. Add to Company in MongoDB using centralized API_BASE_URL
            const res = await fetch(`${API_BASE_URL}/companies/${company.id}/members`, {
                method: 'POST',
