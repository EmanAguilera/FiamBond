...
export default function CreateFamilyTransactionWidget({ family, onSuccess }: Props) {
    ...
    const submit = async (e?: FormEvent) => {
        ...
        try {
            ...
            // 2. Record Corresponding Personal Transaction (Always an expense for the user)
            // Regardless if it's family income or expense, the money leaves/enters the user's personal wallet
            // but for a family transaction, the funds usually come *from* the user's personal account
            // and go *into* the family/are spent by the family. We record it as an expense for the user.
            ...
            const perRes = await fetch(`${API_URL}/transactions`, {
                ...
                body: JSON.stringify({
                    user_id: user.uid,
                    family_id: null, // Personal Transaction
                    description: personalDesc,
                    amount: amountVal,
                    // If it's Family Income (Contribution), it's a personal *Expense* (out of my wallet)
                    // If it's Family Expense, it's a personal *Expense* (out of my wallet)
                    // This logic seems flawed in the original. It should be:
                    // Family Income -> User Expense (Contribution)
                    // Family Expense -> User Expense (Spending)
                    // The original code only sets the personal transaction type to 'expense'.
                    // Let's stick to the original code's intent:
                    type: 'expense', 
                    attachment_url,
                    created_at: timestamp
                })
            ...
            setConflict(null);
            onSuccess?.();
        } catch (e) { toast.error("Error saving transaction"); }
        setLoading(false);
    };

    return (
        <>
            {/* Conflict Modal logic is kept but hidden unless needed */}
            {conflict && (
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" required placeholder={form.type === 'income' ? "e.g. Contribution" : "e.g. Groceries"} value={form.desc} 
                ...
                <p className="text-xs text-center text-slate-400 italic">
                    Note: This also records an expense of the same amount from your personal wallet.
                </p>
            </form>
        </>
    );
